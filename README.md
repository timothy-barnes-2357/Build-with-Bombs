...
1. Copy inference.dll from `inference_dll/build/Release` to `mod_neoforge/native_libraries`.

2. Copy the following NVIDIA DLLs to `mod_neoforge/native_libraries`:
...
3. Copy the model parameter file to `mod_neoforge/native_libaries`.
    * ddim_single_update.onnx (Available from this Github release page)

4. Build the .jar file by running `./gradlew build -Ppack_native_libraries`. This built will take some time. The .jar under `mod_neoforge\build\libs` should be > 1 GB in size. 

5. Verify the binaries were included by running `jar tf /build/libs/buildwithbombs-*.jar` to see all included files. 
